...
@Rule(name="PartnerHistory", priority=Integer.MIN_VALUE)
public class PartnerTerminatedRule {
	...
	@Action(order=1)
	public final void terminateContract(@Fact(PartnerFacts.STAMM) StammImpl stamm,  @Fact(PartnerFacts.RESULTS_CONTRACT_SPEL) Collection<PMContract> contracts,  @Fact(PartnerFacts.RESULTS) Collection<Object> results) {
		Assert.isTrue(contracts.size() <= 1 , "Max. 1 contract can exist.");
		contracts.forEach(contract -> results.add(newTerminatedContract(stamm, contract)));
	}

	private PMContract newTerminatedContract(final StammImpl stamm, final PMContract pmContract) {
	...
	@Action(order=2)
	public final void terminatePartner( @Fact(value = "terminationDates$") FamilyMembers familyMembers, @Fact(PartnerFacts.RESULTS_PARTNERS_CORE_SPEL) Collection<PartnerCore> partnersCore,  @Fact(PartnerFacts.RESULTS) Collection<Object> results) {
		
		partnersCore.forEach(partner-> newTerminatedPartner(familyMembers, partner).ifPresent(value -> results.add(value)));
	}
	...
	@Action(order=3)
	public final void terminatePartnersRole( @Fact(value = "terminationDates$") FamilyMembers familyMembers,  @Fact(PartnerFacts.RESULTS_PARTNERS_ROLE_SPEL) Collection<PartnersRole> partnersroles,  @Fact(PartnerFacts.RESULTS) Collection<Object> results) {
		partnersroles.forEach(role -> newTerminatedPartnerRole(familyMembers, role).ifPresent(value -> results.add(value)));
	}

	private Optional<PartnersRole> newTerminatedPartnerRole(final FamilyMembers familyMembers, final PartnersRole role) {
	...
	private PartnersRole newTerminatedPartnersRole(final PartnersRole role, final Date ind, final Date dop, final long state) {
